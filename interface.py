...
class USBScreen(BaseScreen):
    ...
    def _tarjeta_usb(self, dev):
        ...
        if sospechoso:
            ...
            RoundedButton(card, "Bloquear dispositivo", bg=COLORS["red"], parent_bg=COLORS["card"],
                          font=self.app.f(10, "bold"),
                          command=lambda: messagebox.showinfo(
                              "MemGuard", f"Dispositivo {dev['unidad']} bloqueado.")
                          ).pack(fill="x")
        else:
            tk.Label(card, text="Seguro \u00b7 sin amenazas detectadas",
                     bg=COLORS["card"], fg=COLORS["green"], font=self.app.f(9)).pack(anchor="w", pady=(4, 0))
...
class CiberScreen(BaseScreen):
    def construir(self):
        ...
        tk.Label(consola_wrap, text="Log Kali Linux", bg=COLORS["card"], fg=COLORS["text"],
    ...
    def _log(self, texto):
        ...
        self.consola.insert("end", f"[OK] {texto}\n")
...
class AccesibilidadScreen(BaseScreen):
    def construir(self):
        ...
        tk.Label(voz, text="Motor: pyttsx3  \u00b7  Velocidad: 150 wpm", bg=COLORS["card"],
