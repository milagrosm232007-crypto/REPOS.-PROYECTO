...
class USBScreen(BaseScreen):
    def construir(self):
        ...
        # Mientras esta en True, ni el refresco automatico de esta
        # pantalla ni el vigilante de USB en segundo plano deben
        # reconstruir la lista: el resultado del analisis debe quedar
        # visible hasta que el usuario mismo lo cierre.
    ...
    def actualizar(self):
        if self.mostrando_resultado:
            # No se toca la lista mientras se ve un resultado de analisis.
            return
    ...
    # Pasos "en vivo" que se muestran mientras se analiza el contenido
    # real del USB, con el mismo estilo que la consola de "Control de
    # amenazas". El trabajo real (backend.escanear_usb) se hace de una
    # sola vez antes de animar, para que el resultado final coincida
    # siempre con lo que se encontro de verdad.
    _PASOS_ANALISIS_USB = (
        "Verificando autorun.inf...",
        "Recorriendo carpetas del dispositivo...",
        "Comparando firmas de archivos...",
        "Buscando malware conocido...",
        "Revisando extensiones dobles...",
    )
    ...
    def _tarjeta_usb(self, dev):
        ...
        # Zona donde aparece la consola animada al pedir un analisis;
        # queda vacia hasta que el usuario presiona el boton.
        zona_analisis = tk.Frame(card, bg=COLORS["card"])
        zona_analisis.pack(fill="x")
        ...
        RoundedButton(botones, "Analizar dispositivo", bg=COLORS["blue"], parent_bg=COLORS["card"],
                      ...
                      command=lambda d=dev, z=zona_analisis, b=botones: self._analizar(d, z, b)
    ...
    def _analizar(self, dev, zona, botones):
        self.cancelar_refresco()
        self.mostrando_resultado = True
        botones.pack_forget()  # oculta los botones mientras se analiza

        cabecera = tk.Frame(zona, bg=COLORS["card"])
        cabecera.pack(fill="x", pady=(4, 4))
        tk.Label(cabecera, text="Analisis del dispositivo", bg=COLORS["card"], fg=COLORS["text"],
                 font=self.app.f(9, "bold")).pack(side="left")
        lbl_estado = tk.Label(cabecera, text="Escaneando\u2026", bg=COLORS["card"],
                               fg=COLORS["blue"], font=self.app.f(8, "bold"))
        lbl_estado.pack(side="right")

        barra = ttk.Progressbar(zona, length=100, mode="determinate")
        barra.pack(fill="x", pady=(0, 6))

        consola = tk.Text(zona, bg=COLORS["console_bg"], fg=COLORS["console_text"],
                           font=("Consolas", 9), height=7, relief="flat")
        consola.pack(fill="x")
        consola.tag_configure("alerta", foreground=COLORS["orange"])
        consola.tag_configure("exito", foreground=COLORS["green"])
        consola.configure(state="disabled")

        def log(texto, prefijo="\u2713", tag=None):
            consola.configure(state="normal")
            consola.insert("end", f"{prefijo} {texto}\n", tag) if tag else \
                consola.insert("end", f"{prefijo} {texto}\n")
            consola.see("end")
            consola.configure(state="disabled")

        log("Iniciando analisis del dispositivo...", "\u25b6")

        ctx = {"zona": zona, "barra": barra, "consola": consola, "log": log, "lbl_estado": lbl_estado}
        threading.Thread(target=self._analizar_en_hilo, args=(dev, ctx), daemon=True).start()

    def _analizar_en_hilo(self, dev, ctx):
        resultado = backend.escanear_usb(dev["unidad"])
        self.after(0, lambda: self._animar_pasos_usb(0, resultado, ctx))

    def _animar_pasos_usb(self, indice, resultado, ctx):
        total = len(self._PASOS_ANALISIS_USB)
        if indice < total:
            ctx["log"](self._PASOS_ANALISIS_USB[indice], "\u2713", "exito")
            ctx["barra"]["value"] = int((indice + 1) / total * 100)
            self.after(250, lambda: self._animar_pasos_usb(indice + 1, resultado, ctx))
        else:
            self._finalizar_analisis_usb(resultado, ctx)

    def _finalizar_analisis_usb(self, resultado, ctx):
        if resultado["sospechoso"]:
            for h in resultado["hallazgos"]:
                ctx["log"](h, "\u26a0", "alerta")
            ctx["log"](f"{len(resultado['hallazgos'])} hallazgo(s) sospechoso(s).", "\u274c", "alerta")
            ctx["lbl_estado"].configure(text="\U0001f6a8 Sospechoso", fg=COLORS["red"])
        else:
            ctx["log"](f"Se analizaron {resultado.get('archivos_analizados', 0)} archivo(s).", "\u2713", "exito")
            ctx["log"]("No se detectaron amenazas.", "\u2714", "exito")
            ctx["lbl_estado"].configure(text="\U0001f6e1 Dispositivo seguro", fg=COLORS["green"])

        RoundedButton(ctx["zona"], "Cerrar resultado", bg=COLORS["blue"], parent_bg=COLORS["card"],
                      font=self.app.f(9, "bold"), command=self._volver_a_lista
                      ).pack(fill="x", pady=(8, 0))

        # El resultado se queda en pantalla hasta que el usuario mismo
        # presione "Cerrar resultado": ni el refresco automatico de esta
        # pantalla ni el vigilante de USB en segundo plano lo van a tapar.
...
# --------------------------------------------------------------------------
# Pantalla 5: Control de amenazas (ex "Ciberseguridad")
# --------------------------------------------------------------------------
class CiberScreen(BaseScreen):
    def construir(self):
        self.encabezado("Control de amenazas")
...
class ConfigScreen(BaseScreen):
    def construir(self):
        ...
        # Esta pantalla puede tener mucho contenido (mas aun con el modo
        # desarrollador activo), asi que se envuelve en un canvas con
        # scroll: sin esto, las secciones de mas abajo quedan ocultas
        # fuera de la ventana en pantallas chicas, aunque el codigo este
        # bien y los widgets se hayan creado correctamente.
        contenedor = tk.Frame(self, bg=COLORS["bg"])
        contenedor.pack(fill="both", expand=True)

        canvas = tk.Canvas(contenedor, bg=COLORS["bg"], highlightthickness=0)
        scrollbar = ttk.Scrollbar(contenedor, orient="vertical", command=canvas.yview)
        self.cuerpo = tk.Frame(canvas, bg=COLORS["bg"])

        self.cuerpo.bind(
            "<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all"))
        )
        ventana_canvas = canvas.create_window((0, 0), window=self.cuerpo, anchor="nw")
        canvas.configure(yscrollcommand=scrollbar.set)
        canvas.bind("<Configure>", lambda e: canvas.itemconfigure(ventana_canvas, width=e.width))

        canvas.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")

        # Scroll con la rueda del mouse, solo mientras el cursor esta
        # sobre esta pantalla (para no interferir con otras pantallas).
        def _rueda(event):
            paso = -1 if (event.num == 4 or event.delta > 0) else 1
        ...
        card = Card(self.cuerpo, padx=20, pady=20)
        ...
        botones = tk.Frame(self.cuerpo, bg=COLORS["bg"])
        ...
        seguridad = Card(self.cuerpo, padx=20, pady=20)
        ...
        # --- Modo desarrollador ---
        dev_card = Card(self.cuerpo, padx=20, pady=20)
        dev_card.pack(fill="x", padx=30, pady=(0, 30))
        fila_dev = tk.Frame(dev_card, bg=COLORS["card"])
        fila_dev.pack(fill="x")
        tk.Label(fila_dev, text="Modo desarrollador", bg=COLORS["card"], fg=COLORS["text"],
                 font=self.app.f(12, "bold")).pack(side="left")
        activo = bool(self.app.config_datos.get("modo_desarrollador", False))
        ToggleSwitch(fila_dev, activo, self._alternar_dev).pack(side="right")
        tk.Label(dev_card, text="Habilita herramientas para diagnosticar el rendimiento real "
                                 "del sistema y forzar escaneos manuales desde otras pantallas.",
                 bg=COLORS["card"], fg=COLORS["text_muted"], font=self.app.f(9),
                 anchor="w", justify="left", wraplength=600).pack(anchor="w", pady=(6, 0))

        if activo:
            tk.Frame(dev_card, bg=COLORS["border"], height=1).pack(fill="x", pady=14)
            tk.Label(dev_card, text="Herramientas de desarrollador", bg=COLORS["card"], fg=COLORS["text"],
                     font=self.app.f(10, "bold")).pack(anchor="w", pady=(0, 8))

            RoundedButton(dev_card, "Ejecutar diagnostico de rendimiento ahora", bg=COLORS["blue"],
                          parent_bg=COLORS["card"], font=self.app.f(10, "bold"),
                          command=self.ejecutar_diagnostico).pack(fill="x", pady=(0, 8))
            RoundedButton(dev_card, "Ejecutar prueba de estres (5s)", bg=COLORS["orange"],
                          parent_bg=COLORS["card"], font=self.app.f(10, "bold"),
                          command=self.ejecutar_estres).pack(fill="x", pady=(0, 8))
            RoundedButton(dev_card, "Medir latencia de red", bg=COLORS["green"],
                          parent_bg=COLORS["card"], font=self.app.f(10, "bold"),
                          command=self.medir_latencia).pack(fill="x")

            self.lbl_resultado_dev = tk.Label(
                dev_card, text="Sin diagnosticos ejecutados todavia.", bg=COLORS["card"],
                fg=COLORS["text_muted"], font=("Consolas", 9), anchor="w",
                justify="left", wraplength=600
            )
            self.lbl_resultado_dev.pack(anchor="w", pady=(10, 0))

    def _alternar_dev(self, valor):
        self.app.config_datos["modo_desarrollador"] = valor
        backend.guardar_configuracion(self.app.config_datos)
        # Reconstruye toda la interfaz para que el boton "ejecutar ahora"
        # aparezca/desaparezca tambien en Memoria RAM y Control de amenazas.
        self.app.reconstruir_shell()

    def ejecutar_diagnostico(self):
        self.lbl_resultado_dev.configure(text="Ejecutando diagnostico...")
        threading.Thread(target=self._diagnostico_en_hilo, daemon=True).start()

    def _diagnostico_en_hilo(self):
        resultado = backend.ejecutar_diagnostico_rendimiento()
        self.after(0, lambda: self.lbl_resultado_dev.configure(
            text=(f"[{resultado['hora']}] Diagnostico completo en {resultado['duracion_ms']} ms  \u00b7  "
                  f"CPU {resultado['cpu']:.0f}%  \u00b7  RAM {resultado['ram_percent']:.0f}%  \u00b7  "
                  f"{resultado['procesos_analizados']} procesos  \u00b7  {resultado['amenazas']} amenaza(s)")
        ))

    def medir_latencia(self):
        self.lbl_resultado_dev.configure(text="Midiendo latencia de red\u2026")
        threading.Thread(target=self._latencia_en_hilo, daemon=True).start()

    def _latencia_en_hilo(self):
        resultado = backend.medir_latencia()
        if resultado["exito"]:
            texto = (f"[{resultado['hora']}] Latencia a {resultado['host']}: "
                      f"{resultado['promedio_ms']} ms \u00b7 {resultado['perdida_percent']}% "
                      f"perdida \u00b7 calidad {resultado['calidad']}")
        else:
            texto = f"[{resultado['hora']}] No se pudo medir la latencia: {resultado['mensaje']}"
        self.after(0, lambda: self.lbl_resultado_dev.configure(text=texto))

    def ejecutar_estres(self):
        confirmar = messagebox.askyesno(
            "MemGuard \u2014 Modo desarrollador",
            "Esto va a exigir la CPU al maximo durante 5 segundos en varios nucleos, "
            "para poder ver el efecto en Memoria RAM / Control de amenazas.\n\n"
            "\u00bfContinuar?"
        )
        if not confirmar:
            return
        self.lbl_resultado_dev.configure(text="Ejecutando prueba de estres (5s)...")
        threading.Thread(target=self._estres_en_hilo, daemon=True).start()

    def _estres_en_hilo(self):
        resultado = backend.ejecutar_prueba_estres(5)
        self.after(0, lambda: self.lbl_resultado_dev.configure(
            text=(f"Prueba de estres finalizada \u00b7 {resultado['segundos']}s en "
                  f"{resultado['nucleos_usados']} nucleo(s). Revisa Memoria RAM o "
                  f"Control de amenazas para ver el impacto.")
        ))
